defmodule RetWeb.Plugs.AddCSP do
  ...
  defp generate_csp_rules(strict: false) do
    ...
    ret_direct_connect =
      if is_subdomain do
        "https://#{ret_domain}:#{ret_port} wss://#{ret_domain}:#{ret_port} https://*.#{ret_domain}:#{ret_port} wss://*.#{ret_domain}:#{ret_port} wss://*.#{ret_domain}:#{janus_port} https://*.#{ret_domain}:#{janus_port} #{default_janus_csp_rule}"
      else
