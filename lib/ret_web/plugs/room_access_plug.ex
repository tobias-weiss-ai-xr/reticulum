defmodule RetWeb.Plugs.RoomAccess do
  ...
  Extracts the Bearer token from the Authorization header, verifies it via Guardian
  (same HS512 signer as session tokens), then checks for room-specific claims:
  - `room_id`: the room being accessed (required)
  - `sub`: the user_id (from Guardian's subject_for_token)
